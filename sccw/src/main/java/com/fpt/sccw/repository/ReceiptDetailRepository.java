...
public interface ReceiptDetailRepository extends JpaRepository<ReceiptDetail, Long> {
    ...
    @Query(value = "SELECT d FROM ReceiptDetail d " +
            ...
            countQuery = "SELECT COUNT(d) FROM ReceiptDetail d JOIN d.receipt r JOIN d.product p LEFT JOIN r.user u LEFT JOIN r.assignedUser au " +
                    "WHERE (:warehouseId IS NULL OR r.warehouse.id = :warehouseId) " +
                    "AND (:type IS NULL OR r.type = :type) " +
                    "AND (:search IS NULL OR (LOWER(p.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(p.code) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(COALESCE(r.partner, '')) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(CONCAT('r-', r.id)) LIKE LOWER(CONCAT('%', :search, '%')) OR CONCAT('', r.id) LIKE CONCAT('%', :search, '%') OR LOWER(COALESCE(u.fullName, '')) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(COALESCE(au.fullName, '')) LIKE LOWER(CONCAT('%', :search, '%')))) " +
                    "AND (:status IS NULL OR r.status = :status) " +
